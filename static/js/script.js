...
errorWindow
	...
	.addEventListener("click", (_) => errorWindow.classList.remove("active"));
...
clsBtnAnalyticsWindow.addEventListener("click", (_) => {
	analyticWindow.classList.contains("inactive")
		? ""
		: analyticWindow.classList.add("inactive");
...
desBtns[0].addEventListener("click", (_) => {
	...
	viewBtns[0].addEventListener("click", (_) => {
		...
		proccessWindow.classList.contains("process")
			? proccessWindow.classList.remove("process")
			: proccessWindow.classList.add("");
...
upldImg1.addEventListener("input", (_) => {
	...
	if (file) {
		...
		reader.onload = (_) => {
			...
			rsllImgContainer.classList.contains("inactive")
				? ""
				: rsllImgContainer.classList.add("inactive");
		};
		rmvBtns[0].addEventListener("click", (_) => {
			...
			rsllImgContainer.classList.contains("inactive")
				? ""
				: rsllImgContainer.classList.add("inactive");
			hybridControlBtns[1].classList.contains("process")
				? hybridControlBtns[1].classList.remove("process")
				: "";
			activeCombineClearBtns();
		});
		reader.readAsDataURL(file);
...
hybridControlBtns[0].addEventListener("click", (_) => {
	...
	img1.classList.remove("active");
	img2.classList.remove("active");
	rmvBtns[0].classList.contains("active")
		? rmvBtns[0].classList.remove("active")
		: "";
	rmvBtns[1].classList.contains("active")
		? rmvBtns[1].classList.remove("active")
		: "";
	rsllImgContainer.classList.contains("inactive")
		? ""
...
function activeCombineClearBtns() {
	img1Stat && img2Stat
		? hybridControlBtns[1].classList.remove("inactive")
		: hybridControlBtns[1].classList.contains("inactive")
		? ""
		: hybridControlBtns[1].classList.add("inactive");
	img1Stat || img2Stat
		? hybridControlBtns[0].classList.remove("inactive")
		: hybridControlBtns[0].classList.contains("inactive")
		? ""
		: hybridControlBtns[0].classList.add("inactive");
}
...
function rescieveHistograam(histo) {
	...
	fetch(`${window.origin}/recieve_histogram`, {
		...
		response.json().then((data) => {
			...
			analyticWindow.querySelector(".dis_graph").classList.contains("inactive")
				? ""
				: analyticWindow.querySelector(".dis_graph").classList.add("inactive");
			analyticWindow.querySelector(".histo").classList.contains("inactive")
				? analyticWindow.querySelector(".histo").classList.remove("inactive")
				: "";
...
function recieveEqHistogram(histo) {
	...
	fetch(`${window.origin}/recieve_rgb`, {
		...
		response.json().then((data) => {
			...
			eqCmlWindow.querySelector(".eq").classList.contains("inactive")
				? eqCmlWindow.querySelector(".eq").classList.remove("inactive")
				: "";
			eqCmlWindow.querySelector(".cml").classList.contains("inactive")
				? ""
				: eqCmlWindow.querySelector(".cml").classList.add("inactive");
			eqCmlWindow.classList.contains("inactive")
				? eqCmlWindow.classList.remove("inactive")
				: "";
			proccessWindow.classList.remove("active");
			proccessWindow.classList.remove("dec");
			proccessWindow.classList.add("act");
...
function recieveDistData(dist) {
	...
	fetch(`${window.origin}/recieve_distribution_curve`, {
		...
		response.json().then((data) => {
			...
			analyticWindow.querySelector(".histo").classList.contains("inactive")
				? ""
				: analyticWindow.querySelector(".histo").classList.add("inactive");
			analyticWindow.querySelector(".dis_graph").classList.contains("inactive")
				? analyticWindow
...
function recieveCmlCurve(dist) {
	...
	fetch(`${window.origin}/recieve_cml_curve`, {
		...
		response.json().then((data) => {
			...
			proccessWindow.classList.remove("active");
			eqCmlWindow.querySelector(".cml").classList.contains("inactive")
				? eqCmlWindow.querySelector(".cml").classList.remove("inactive")
				: "";
			eqCmlWindow.querySelector(".eq").classList.contains("inactive")
				? ""
				: eqCmlWindow.querySelector(".eq").classList.add("inactive");
		});
	});
}
...
actBtns.forEach((ele) => {
	ele.addEventListener("click", (_) => {
		...
		imgTestChoose.querySelector(
			".t"
		).innerText = `Choose an image as a test sample to try ${ele.innerHTML
			.split(" ")
			.slice(1, ele.innerHTML.split(" ").length)
			.join(" ")}`;
		tryBtn.querySelector("p").innerText = `Try ${ele.innerHTML
			.split(" ")
			.slice(1, ele.innerHTML.split(" ").length)
			.join(" ")}`;
...
bkBtn.addEventListener("click", (_) => {
	...
	startProBtn.classList.contains("active")
		? startProBtn.classList.remove("active")
		: "";
	document
