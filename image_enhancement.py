...
def draw_histogram(im_array, color):
...
def rgb_mode(im):  # This is what we call
    image = cv2.imread(im)
    rgb = rgb_to_gray(image, 0)
    rgb_array = []
    # color_list=['r','g','b']
    for i in rgb:
...
def gray_scale_mode(img_path):  # This is what we call
    unique, count = np.unique(cv2.imread(img_path, 0), return_counts=True)
    return unique, count


def draw_distribution(img):
...
# draw_distribution('./static/download/edit/bl.jpg')


def image_equalization(img, bin):
    image = cv2.imread(img)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    flat = image.flatten()
