...
import image_filters as fn
import image_histograms as hs
import image_enhancement as fr2
import global_local_thresholding as fr1
import corner_detection as crn
import active_contour as ct
import hough_implementation as hf
import cv2
import ellipse_detection as ed
import ssd_ncc_implementaion
...
def handle_filter(img_path, filter_name):
    ...
    if filter_name == 'gaussian_noise':
        ...
        new_path_img = fr2.image_normalization(img_path)
    elif filter_name == "equalizer":
        new_path_img = fr2.image_equalization(img_path, 256)
...
def ssd_ncc_recive():
    if request.method == 'POST':
        ...
        with open(upld_img2_file, 'wb') as f:
            f.write(upld_img2)
        mode_time, img_rslt, sift_time = ssd_ncc_implementaion.feature_matching(
...
def face_recognition():
    if request.method == 'POST':
        ...
        data = roc_implementation.Visualization(upld_img_file, int(req['thr']))
